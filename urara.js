...
const output = (color, msg, filepath) =>
	console.log(
		`${chalk.dim(new Date().toLocaleTimeString())} ${chalk.magenta.bold(`[urara]`)} ${chalk[color](
			msg
...
async function mkDir(dir) {
	await fs.mkdir(dir, { recursive: true }).then(() =>
		output('yellow', 'make dir', dir)
	)
}

async function rmDir(dir) {
	await fs.rm(dir, { recursive: true }).then(() =>
		output('red', 'remove dir', dir)
	)
}
	...
	await fs.readdir(dir, { withFileTypes: true }).then(files => {
		for (let file of files) {
			if (
				...
				fs.rm(path.join(dir, file.name), { recursive: true }).then(() =>
					output('red', 'remove dir', path.join(dir, file.name))
				)
		}
	})
}
			...
			if (file.isDirectory()) {
				...
				; /\.(svelte|svx|md|js|ts)$/i.test(file.name)
					? fs
						...
						.then(() =>
							output('green', 'copy file', path.join(`src/routes`, dir.slice(6), file.name))
						)
					: fs
...
async function watchAdd(file, unlink = false) {
	let pathAdd
		; /\.(svelte|svx|md|js|ts)$/i.test(path.parse(file).ext)
			? (pathAdd = `src/routes`)
			: (pathAdd = `static`)
	if (unlink === true)
